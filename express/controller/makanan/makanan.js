...
module.exports = {
  async search(req, res) {
    ...
    console.log("search makanan", pages, limit);
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "something happen when fetching makanan" });
    }
  },
  async addTofavorit(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "something happen when adding makanan" });
    }
  },
  async removeFavorit(req, res) {
    ...
    try {
      ...
      return res
        .status(200)
        .send({ message: "remove favorite makanan successful" });
    } catch (e) {
      console.log(e.message);
      return res
        .status(500)
        .send({ message: "something happen when removing favorite" });
    }
  },
  ...
  async update(req, res) {
    ...
    try {
      ...
      if (!status[0])
        return res.status(404).send({ message: "makanan not found" });

      return res.status(200).send({ message: "update makanan succesfull" });
    } catch (e) {
      console.log(e.message);
      return res
        .status(500)
        .send({ message: "something happen when updating makanan" });
    }
  },
  ...
  async deleteOne(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "something happen when deleting makanan" });
    }
  },
  ...
  async create(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "something happen when creating Makanan !" });
    }
  },
};
