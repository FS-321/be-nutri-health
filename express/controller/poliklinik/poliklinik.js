...
module.exports = {
  async search(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "something happen when fetching poliklinik" });
    }
  },
  async create(req, res) {
    ...
    try {
      ...
      return res.status(200).send({ message: "creating poliklinik success" });
    } catch (e) {
      console.log(e.message);
      return res
        .status(500)
        .send({ message: "something happen when creating poliklinik" });
    }
  },
  ...
  async update(req, res) {
    ...
    try {
      ...
      return res.status(200).send({ message: "Update poliklinik succseful" });
    } catch (e) {
      return res
        .status(500)
        .send({ message: "something happen when updating poliklinik" });
    }
  },
  ...
  async deleteOne(req, res) {
    ...
    try {
      ...
      if (!status)
        return res.status(404).send({ message: "poliklinik not found" });
      return res.status(200).send({ message: "delete poliklinik succseful" });
    } catch (e) {
      return res
        .status(500)
        .send({ message: "something happen when deleting poliklinik" });
    }
  },
};
