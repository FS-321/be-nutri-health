...
module.exports = {
  async search(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "something happen when fetching pasien" });
    }
  },
  async create(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "something happen when creating rekaman" });
    }
  },
  async getAll(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .send({ message: "something happen when fetching rekam_medis" });
    }
  },
  ...
  async getOne(req, res) {
    ...
    try {
      const rekam_medis = await DataRekamMedis.findOne({
        where: { data_rekam_id: id },
      });
  ...
  async getOneUser(req, res) {
    ...
    try {
      const rekam_medis = await DataRekamMedis.findOne({
        where: { data_rekam_id: id, pasien_id },
      });
  ...
  async update(req, res) {
    ...
    try {
      const status = await DataRekamMedis.update(data, {
        where: { data_rekam_id: id },
      });

      return res.status(200).send({ message: "Update rekam medis successful" });
    } catch (e) {
      return res
        .status(500)
        .send({ message: "something happen when updating rekam_medis" });
    }
  },
  ...
  async deleteOne(req, res) {
    const id = req.params.id;
    try {
      const status = await DataRekamMedis.destroy({
        where: { data_rekam_id: id },
      });

      if (!status)
        return res.status(404).send({ message: "rekam_medis not found" });
      return res.status(200).send({ message: "delete succseful" });
    } catch (e) {
      return res
        .status(500)
        .send({ message: "something happen when deleting rekam_medis" });
    }
  },
};
