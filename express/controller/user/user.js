...
module.exports = {
  async search(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "something happen when fetching user" });
    }
  },
  ...
  async create(req, res) {
    ...
    try {
      ...
      return res
        .stats(500)
        .send({ message: "something happen when creating user" });
    }
  },
  ...
  async getOne(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: "Something error when fetchind user" });
    }
  },
  ...
  async update(req, res) {
    ...
    const updated_user = req.params.id
      ? req.params.id
      : getDecodedToken(req, res)["user_id"];

    try {
      const status = await User.update(data, {
        where: { user_id: updated_user },
      });
      ...
      return res
        .status(500)
        .send({ message: "something happen when updating user" });
    }
  },
